...
class MarcatWidgetTriggerItemTopix02 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '新着情報【Newアイコン・日付（年月日）・カテゴリー・タイトル】', 'text_domain' ), // Name
            array( 'description' => __( '新着情報【Newアイコン・日付（年月日）・カテゴリー・タイトル】の新着情報を作成します。', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <div class="topics_box_ichigyou">
            <?php if(!empty($posts)): ?>
                <ul class="topics_loop">
                    <?php foreach($posts as $post): ?>
                        ...
                        <li>
                            <a href="<?php echo get_the_permalink($post->ID); ?>">
                                ...
                                <span class="category"><span class="slug_<?php if(!empty($cat_slug)){ echo $cat_slug; } ?>"><?php if(!empty($cat_slug)){ echo strtoupper($cat_slug); } ?></span></span>
                                <span class="title_box"><?php echo get_the_title($post->ID); ?></span>
                            </a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            ...
            <div class="link_box">
                <?php if(!$post_type_hira==='post'): ?>
                    <a href="<?php echo home_url('/'); ?><?php echo $post_type_hira; ?>/"><?php echo $link_pref; ?></a>
                <?php else: ?>
                    <?php if($cat_id_hira == "null" or $cat_id_hira == ""): ?>
                    <?php else: ?>
                        <a href="<?php echo get_category_link($cat_id_hira); ?>"><?php echo $link_pref_hira; ?></a>
                    <?php endif; ?>
                <?php endif; ?>
            </div>
        </div>
    ...
    function form($instance) {
        ...
        <p>
          ...
          <select name="<?php echo $this->get_field_name('cat_id_hira'); ?>" size="">
            <?php
                $args = array('orderby' => 'order','order' => 'ASC');
                $cat_all = get_categories($args);
            ?>
            <option value="null" <?php if(!empty($cat_id_hira)){ if($cat_id_hira==$value->cat_ID){ echo 'selected'; } } ?>>--選択なし--
                </option>
...
function List_MarcatWidgetTriggerItemTopix02() {
    register_widget( 'MarcatWidgetTriggerItemTopix02' );
}
add_action( 'widgets_init', 'List_MarcatWidgetTriggerItemTopix02' );
?>