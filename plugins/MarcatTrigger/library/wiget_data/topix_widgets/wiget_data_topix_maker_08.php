...
/*■新着情報【サムネイルのみ】■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■*/
class MarcatWidgetTriggerItemTopix08 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '【ウェブチ　コンテンツ No.007～010】', 'text_domain' ), // Name
            array( 'description' => __( '【ウェブチ　コンテンツ No.007～010】を使用する、バナー掲載（サムネイル）のみのコンテンツ', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <section class="wiget_data_topix_maker_08">
            <?php if(!empty($posts)): ?>
                <ul class="wiget_data_topix_maker_08_topics_loop wapper_column_<?php echo $postnum_hira; ?>">
                        ...
                        ?>
                          <li>
                            <?php if(empty($customdata['webchi_google_map_code'])): ?>
                                <div class="wiget_data_topix_maker_08_thumbs">
                            ...
                            <?php else: ?>
                                <div class="wiget_data_topix_maker_08_google_code" style="padding:<?php if(!empty($customdata['webchi_google_paddings'])){ echo $customdata['webchi_google_paddings'] ;} else{ echo 0; } ?>% 0;">
...
function ListMarcatWidgetTriggerItemTopix08() {
    register_widget( 'MarcatWidgetTriggerItemTopix08' );
}
add_action( 'widgets_init', 'ListMarcatWidgetTriggerItemTopix08' );
?>