...
/*■新着情報【サムネイルのみ】■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■*/
class MarcatWidgetTriggerItemTopix11 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '【ウェブチ　コンテンツ No.014】', 'text_domain' ), // Name
            array( 'description' => __( '【ウェブチ　コンテンツ No.014】を使用する、サムネイルと文章が載るものです。', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <section class="wiget_data_topix_maker_11">
            <?php if(!empty($posts)): ?>
                <ul class="wiget_data_topix_maker_11_topics_loop wapper_column_<?php echo $postnum_hira; ?>">
                        ...
                        <li>
                            <div class="left_box">
                                <?php if(empty($customdata['webchi_google_map_code'])): ?>
                                    <div class="wiget_data_topix_maker_11_thumbs">
                                ...
                                <?php else: ?>
                                    <div class="wiget_data_topix_maker_11_google_code" style="padding:<?php if(!empty($customdata['webchi_google_paddings'])){ echo $customdata['webchi_google_paddings'] ;} else{ echo 0; } ?>% 0;">
                                        <?php echo $customdata['webchi_google_map_code']; ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <section class="wiget_data_topix_maker_11_pref">
                                <p><?php $post = get_post( $post->ID );  echo $post->post_content; ?></p>
                            </section>
                        </li>
                    <?php $i++; endforeach; ?>
                </ul>
...
function ListMarcatWidgetTriggerItemTopix11() {
    register_widget( 'MarcatWidgetTriggerItemTopix11' );
}
add_action( 'widgets_init', 'ListMarcatWidgetTriggerItemTopix11' );
?>