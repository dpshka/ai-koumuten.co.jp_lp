...
/*■新着情報【サムネイルのみ】■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■*/
class MarcatWidgetTriggerItemTopix10 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '【ウェブチ　コンテンツ No.013】', 'text_domain' ), // Name
            array( 'description' => __( '【ウェブチ　コンテンツ No.013】を使用する、4カララム掲載　バナー掲載（サムネイル）のみのコンテンツ', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <section class="wiget_data_topix_maker_10">
            <?php if(!empty($posts)): ?>
                <ul class="wiget_data_topix_maker_10_topics_loop wapper_column_<?php echo $postnum_hira; ?>">
                    <?php $i=1; foreach($posts as $post): $customdata = marcatgia($post->ID); ?>
                        ...
                        ?>
                        <?php if($i===1 or $i%4===1) : ?><div class="wiget_data_topix_maker_10_list_wapper"><?php endif; ?>
                        <li>
                            <?php if(empty($customdata['webchi_google_map_code'])): ?>
                                <div class="wiget_data_topix_maker_10_thumbs">
                            ...
                            <?php else: ?>
                                <div class="wiget_data_topix_maker_10_google_code" style="padding:<?php if(!empty($customdata['webchi_google_paddings'])){ echo $customdata['webchi_google_paddings'] ;} else{ echo 0; } ?>% 0;">
...
function ListMarcatWidgetTriggerItemTopix10() {
    register_widget( 'MarcatWidgetTriggerItemTopix10' );
}
add_action( 'widgets_init', 'ListMarcatWidgetTriggerItemTopix10' );
?>