...
/*■新着情報【サムネイルのみ】■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■*/
class MarcatWidgetTriggerItemTopix011 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '【ウェブチ　コンテンツ No.011】', 'text_domain' ), // Name
            array( 'description' => __( '【ウェブチ　コンテンツ No.011】を使用する、2カララム掲載　バナー掲載（サムネイル）のみのコンテンツ', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <section class="wiget_data_topix_maker_011">
            <?php if(!empty($posts)): ?>
                <ul class="wiget_data_topix_maker_011_topics_loop wapper_column_<?php echo $postnum_hira; ?>">
                    <?php $i=1; foreach($posts as $post): $customdata = marcatgia($post->ID); ?>
                        ...
                        ?>
                        <?php if($i%2===1) : ?><div class="wiget_data_topix_maker_011_list_wapper"><?php endif; ?>
                        <li>
                            <?php if(empty($customdata['webchi_google_map_code'])): ?>
                                <div class="wiget_data_topix_maker_011_thumbs">
                            ...
                            <?php else: ?>
                                <div class="wiget_data_topix_maker_011_google_code" style="padding:<?php if(!empty($customdata['webchi_google_paddings'])){ echo $customdata['webchi_google_paddings'] ;} else{ echo 0; } ?>% 0;">
...
function ListMarcatWidgetTriggerItemTopix011() {
    register_widget( 'MarcatWidgetTriggerItemTopix011' );
}
add_action( 'widgets_init', 'ListMarcatWidgetTriggerItemTopix011' );
?>