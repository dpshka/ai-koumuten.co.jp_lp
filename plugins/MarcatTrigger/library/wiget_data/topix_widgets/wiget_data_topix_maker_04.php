...
class MarcatWidgetTriggerItemTopix04 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '新着情報【左に｛サムネイル｝・右に｛Newアイコン・日付（年月日）・タイトル・本文（抜粋）｝】', 'text_domain' ), // Name
            array( 'description' => __( '新着情報【左：サムネイル<br>右：Newアイコン・日付（年月日）・タイトル・本文（抜粋）】の新着情報を作成します。これには、タイトルとサブタイトルに加えて、一覧へのリンクも作成できるようになっています。', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <section class="wiget_data_topix_maker_04">
            <?php if(!empty($posts)): ?>
                <ul class="wiget_data_topix_maker_04_topics_loop">
                    <?php foreach($posts as $post): ?>
                        ...
                        ?>
                        <li>
                            <div class="wiget_data_topix_maker_04_thumbs">
                                <a href="<?php echo get_the_permalink($post->ID); ?>">
                                    <?php echo get_the_post_thumbnail( $post->ID, 'medium', 'wiget_data_topix_maker_04_thumbs_img' ); ?>
                                </a>
                            </div>
                            <a href="<?php echo get_the_permalink($post->ID); ?>">
                                <div class="wiget_data_topix_maker_04_date_box">
                                ...
                                <h2 class="wiget_data_topix_maker_04_post_title"><?php echo titletagnasi($post->ID,'15'); ?></h2>
                                <p class="wiget_data_topix_maker_04_post_pref">
                                    <?php echo honbuntagnasi($post->ID,'100'); ?>
                                </p>
                            </a>
                        </li>
                    <?php endforeach; ?>
            ...
            <div class="wiget_data_topix_maker_04_title_link_box">
                <?php if(!$post_type_hira==='post'): ?>
                    <a href="<?php echo home_url('/'); ?><?php echo $post_type_hira; ?>/">
                        <span class="wiget_data_topix_maker_04_title_link_arrow"><?php echo $link_pref_hira; ?></span>
                    </a>
                <?php else: ?>
...
function ListMarcatWidgetTriggerItemTopix04() {
    register_widget( 'MarcatWidgetTriggerItemTopix04' );
}
add_action( 'widgets_init', 'ListMarcatWidgetTriggerItemTopix04' );
?>