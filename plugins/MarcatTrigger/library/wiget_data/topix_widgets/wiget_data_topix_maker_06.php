...
class MarcatWidgetTriggerItemTopix06 extends WP_Widget {
    function __construct() {
        parent::__construct(
            '', // Base ID
            __( '【ウェブチ　コンテンツ No.002～No.005】', 'text_domain' ), // Name
            array( 'description' => __( '【ウェブチ　コンテンツ No.002～No.005】で使用する上部にサムネイル、下にテキストが来る新着情報一覧フィード', 'text_domain' ), )
        );
    }
    function widget($args, $instance) {
        ...
        ?>
        <section class="wiget_data_topix_maker_06">
            <?php if(!empty($posts)): ?>
                <ul class="wiget_data_topix_maker_06_topics_loop wapper_column_<?php echo $postnum_hira; ?>">
                    <?php foreach($posts as $post): $customdata = marcatgia($post->ID); ?>
                        <?php
                        ...
                        <li>
                            <?php if(empty($customdata['webchi_google_map_code'])): ?>
                                <div class="wiget_data_topix_maker_06_thumbs">
                            ...
                            <?php else: ?>
                                <div class="wiget_data_topix_maker_06_google_code" style="padding:<?php if(!empty($customdata['webchi_google_paddings'])){ echo $customdata['webchi_google_paddings'] ;} else{ echo 0; } ?>% 0;">
                                    <?php echo $customdata['webchi_google_map_code']; ?>
                                </div>
                            <?php endif; ?>
                            <p class="wiget_data_topix_maker_06_post_pref">
...
function ListMarcatWidgetTriggerItemTopix06() {
    register_widget( 'MarcatWidgetTriggerItemTopix06' );
}
add_action( 'widgets_init', 'ListMarcatWidgetTriggerItemTopix06' );

function honbuntagnasi_rest_cheanges($postid, $length){
